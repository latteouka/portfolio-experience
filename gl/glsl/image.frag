...
void main(void) {
  vec2 coverUv = getCoverUv(vUv, vec2(1.0, 1.0), vec2(512.0, 512.0));

  // texture
  vec4 color = texture2D(u_texture, coverUv);

  float imageBlobNoise = snoise(vec3(vUv.x * u_spikes, vUv.y * u_spikes, u_time * 0.5));
  float radius = map(imageBlobNoise, 0.0, 1.0, u_radius*0.9, u_radius);
  float mask = circle(vUv, radius);
  color.a = mask * u_alpha;
